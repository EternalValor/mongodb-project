...
class App extends Component {
  ...
  delete = id => {
    axios
      ...
      .then(res => {
        let newPubsState = this.state.publications.filter(
          pub => pub._id !== id
        );
  ...
  update = (e, id, updatedPub) => {
    ...
    axios.put(`/api/publications/${id}`, { ...updatedPub }).then(res => {
      let newPubsState = this.state.publications.map(
        pub => (pub._id === id ? updatedPub : pub)
      );
      this.setState({ publications: newPubsState });
    });
  };
  ...
  addPublication = (e, pub, push) => {
    ...
    axios.post('/api/publications', { ...pub }).then(res => {
  ...
  aggregateTypesCount = () => {
    axios.get('/api/publications/count').then(res => {
      let newTypesCount = [];
  ...
  componentDidMount() {
    ...
    if (token !== null) {
      ...
      axios.post(`/signin/${token}`).then(res => {
        const data = res.data;
