...
const publication = props => {
  ...
  props.expanded
    ? (pubClass = 'publication publication--expanded')
    : (pubClass = 'publication');
    ...
    <div className={pubClass}>
          ...
          <svg
            ...
            onClick={() => {
              props.beingUpdated === props.index
                ? props.update(-1, {})
                : props.update(props.index, props.publication);
      ...
      <h1 className="publication__heading">
        {props.expanded
          ...
          : props.publication.title.length > 50
            ? `${props.publication.title.slice(0, 50)}...`
            : props.publication.title}
      </h1>
      <div
        ...
        {props.expanded ? (
          props.beingUpdated !== props.index ? (
            <div className="mt-2">
              ...
              {Object.keys(props.publication).map(
                (field, index) =>
                  field !== '__v' ? (
                    exclude.indexOf(field) === -1 ? (
                      props.publication[field].length !== 0 ? (
                        field !== 'url' ? (
                          <div key={field} className="publication__body__field">
                            <span className="field-title">{field}</span>
                            :&nbsp;
                            {field !== 'pages'
                              ? typeof props.publication[field] === 'object'
                                ? props.publication[field].join(', ')
                                : props.publication[field]
                              : `${props.publication[field].begin} - ${
                                  props.publication[field].end
                                }`}
                          </div>
                        ) : (
                          <a
                            href={
                              props.publication[field].slice(0, 4) === 'http'
                                ? props.publication[field]
                                : `http://${props.publication[field]}`
                            }
            ...
            <form
              onSubmit={e => {
                e.preventDefault();
                if (props.updatedPub.IF)
                  props.updatedPub.IF = document.querySelector(
                    "input[name='IF']"
                  ).value;
                props.updateHandler(e, props.publication._id, props.updatedPub);
                if (props.beingUpdated === props.index) props.update(-1, {});
                props.showPopup(
                  `${props.updatedPub.title.slice(
                    0,
                    25
                  )}... successfully edited!`,
                  false
              ...
              {Object.keys(props.publication).map(
                (field, index) =>
                  field !== '__v' ? (
                    exclude.indexOf(field) === -1 ? (
                      props.publication[field].length !== 0 ? (
                        <div key={index} className="publication__body__field">
                          <span className="field-title">{field}</span>
                          :&nbsp;
                          {field !== 'pages' ? (
                            typeof props.publication[field] === 'object' ? (
