...
class Search extends React.Component {
  state = {
    ...
    types: {
      ...
      2: {
        name:
          'Publications parues dans des revues internationales à comité de lecture',
        fields: {
          authors: 'text',
          journal: 'text',
          year: 'number'
      ...
      3: {
        name:
          'Publications parues dans des revues nationales à comité de lecture',
        fields: {
          authors: 'text',
          journal: 'text',
          year: 'number'
      ...
      5: {
        name:
          'Chapitres d’ouvrage indexés dans des bases de données internationales',
        fields: {
          chapTitle: 'text',
          authors: 'text',
          publisher: 'text',
      ...
      9: {
        name:
          'Thèses de doctorat soutenues et encadrées par un membre de la structure',
        fields: {
          authors: 'text',
          advisor: 'text',
          jury: 'text',
  ...
  render() {
    const searchContainerClass = this.state.advSearch
      ? 'search-container search-container--hidden'
      : 'search-container';
    const advSearchBtnClass = this.state.advSearch
      ? 'adv-search-btn adv-search-btn--hidden'
      : 'adv-search-btn';
    const advSearchFormClass = this.state.advSearch
      ? 'adv-search-form'
      : 'adv-search-form--hidden';
    const advSearchContainerClass = this.state.advSearch
      ? 'adv-search-container adv-search-container--shown'
      ...
      <div className="header">
        <div className={searchContainerClass}>
          <div
            ...
            &nbsp; MongoProject
            <br />
            <span className="logo-subtext">BD Avancee - UCD</span>
          </div>
          <form
            className="form"
            onSubmit={e => {
          ...
          {this.props.loggedIn ? (
            <span>
              <Link className="add-btn" to="/add-publication">
                + Add
              </Link>
        ...
        <div className={advSearchContainerClass}>
          <div className={advSearchBtnClass}>
            <span onClick={this.toggleAdvSearch} style={{ cursor: 'pointer' }}>
              Advanced Search
            </span>
          </div>
          ...
          <form className={advSearchFormClass} onSubmit={this.onSubmit}>
            ...
            {this.state.selectedType > 0
              ? Object.keys(
                  this.state.types[`${this.state.selectedType}`].fields
                ).map((field, index) => (
                  <div key={index}>
                    <span className="adv-input-title">
                      {field === 'fundingOrganisation' ? 'Organisation' : field}
                      :{' '}
                    </span>
                    {this.state.types[`${this.state.selectedType}`].fields[
                      field
                      ...
                      <span>
                        ...
                        <input
                          ...
                          name={`${field} from`}
                          value={
                            this.state.advQuery[`${field} from`]
                              ? this.state.advQuery[`${field} from`]
                              : ''
                          }
                          onChange={this.onChange}
              ...
              : null}
            <div
              className="adv-form-btn adv-form-btn--orange"
